...
BOOST_AUTO_TEST_SUITE(eosdt_exchang_tests)
    ...
    BOOST_AUTO_TEST_CASE(vesting_2y_pause_base_test)
    try {
        TESTER chain;
        if (!test_necessary(chain, "vesting_2y_pause_base_test")) {
            return;
        }
        ...
        PUSH_CONSOLE(chain, CMKRYVESTING, N(pausevesting), CMKRYVESTING,
                     mvo()("vesting_id",0)("pause_start","2021-01-20T00:00:00"));
        asset = print_cmkasset(chain, ORE_SYMBOL);
        BOOST_CHECK_EQUAL(asset.asset_balance, ORE(24000));
        BOOST_CHECK_EQUAL(asset.unallocated_balance, ORE(0));
        ...
        PUSH_CONSOLE(chain, CMKRYVESTING, N(unpausevestng), CMKRYVESTING,
        mvo()("vesting_id",0)("pause_end","2022-01-02T00:00:00"));
        asset = print_cmkasset(chain, ORE_SYMBOL);
        BOOST_CHECK_EQUAL(asset.asset_balance, ORE(12000));
        BOOST_CHECK_EQUAL(asset.unallocated_balance, ORE(0));

        PUSH_CONSOLE(chain, EOSDTCNTRACT, N(timemove), EOSDTCNTRACT, mvo()("seconds", 86400*(370+365+9)-12));
        ...
        PUSH_CONSOLE(chain, EOSDTCNTRACT, N(timemove), EOSDTCNTRACT, mvo()("seconds", 86400*(370+365+365-22)-12));

        PUSH_CONSOLE(chain, CMKRYVESTING, N(claim), N(employee),mvo()("vesting_id",0));
        BOOST_CHECK_EQUAL(balance_get(chain, CMKRYVESTING, ORE_SYMBOL), ORE(0));
        BOOST_CHECK_EQUAL(balance_get(chain, N(employee), ORE_SYMBOL), ORE(24000));
        asset = print_cmkasset(chain, ORE_SYMBOL);
