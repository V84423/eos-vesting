...
static char *write_value(char *r, const char *l, const std::array <uint8_t, size> &v) {
    if (r + v.size() * 2 > l) {
        return r;
    }
...
template<size_t size>
static char *write_value(char *r, const char *l, const std::array<char, size> &v) {
    if (r + v.size() * 2 > l) {
        return r;
    }
    for (auto i = 0; i < v.size(); i++) {
...
template<size_t size>
static char *write_value(char *r, const char *l, const eosio::fixed_bytes <size> &v) {
    return write_value(r, l, v.extract_as_byte_array());
}
...
static char *write_value(char *r, const char *l, const eosio::public_key &v) {
    eosio::ecc_public_key p = std::get<0>(v);
    return write_value(r, l, p);
}

static char *write_value(char *r, const char *l, const eosio::signature &v) {
    return write_value(r, l, std::get<0>(v));
}
...
static char *write_value(char *r, const char *l, const std::tm &v) {
    if (r + 15 > l) {
        return r;
    }
    r = write_ulong(r, l, (ds_ulong) v.tm_year, 4ULL);
    r = write_ulong(r, l, (ds_ulong) v.tm_mon, 2ULL);
    r = write_ulong(r, l, (ds_ulong) v.tm_mday, 2ULL);
    *(r++) = ' ';
    r = write_ulong(r, l, (ds_ulong) v.tm_hour, 2ULL);
    *(r++) = ':';
    r = write_ulong(r, l, (ds_ulong) v.tm_min, 2ULL);
    *(r++) = ':';
    r = write_ulong(r, l, (ds_ulong) v.tm_sec, 2ULL);
    return r;
}
...
ds_asset op_mul_div(const ds_asset &quantity, const long double mul, const long double div) {
    ...
    return ds_asset(value, quantity.symbol);
}
...
char hex_by_int(int v) {
    if (v >= 0 && v <= 9)return '0' + v;
    if (v >= 10 && v <= 15)return 'a' + v - 10;
    return '!';
}

eosio::checksum160 checksum160_by_hex(const ds_string &hex) {
    ds_assert(hex.size() == 40 && hex.size() % 2 == 0, "Wrong hex '%'(%) for checksum160(20/40)", hex, hex.size());
    eosio::checksum160 result;
    auto a = result.data();
    auto b = (char *) a;
    for (auto i = 0; i < 40; i += 2) {
        auto f = int_by_hex(hex[i]);
        auto s = int_by_hex(hex[i + 1]);
        ds_assert(f != -1 && s != -1, "Wrong hex '%'(%) for checksum160, it contains wrong char pos:'%' or pos:'%'.",
                  hex, hex.size(), i, i + 1);
        auto v = f * 16 + s;
        if (i / 2 < 16) b[15 - i / 2] = v;
        if (i / 2 > 15 && i / 2 < 32) b[47 - i / 2] = v;
    }
    return result;
}

eosio::checksum256 checksum256_by_hex(const ds_string &hex) {
    ds_assert(hex.size() == 64 && hex.size() % 2 == 0, "Wrong hex '%'(%) for checksum256(32/64)", hex, hex.size());
    ...
    for (auto i = 0; i < 64; i += 2) {
        auto f = int_by_hex(hex[i]);
        auto s = int_by_hex(hex[i + 1]);
        ds_assert(f != -1 && s != -1, "Wrong hex '%'(%) for checksum256, it contains wrong char pos:'%' or pos:'%'.",
                  hex, hex.size(), i, i + 1);
        auto v = f * 16 + s;
        if (i / 2 < 16) b[15 - i / 2] = v;
        if (i / 2 > 15 && i / 2 < 32) b[47 - i / 2] = v;
...
eosio::signature signature_rsv_by_hex(const ds_string &hex) {
    ds_assert(hex.size() == 130 && hex.size() % 2 == 0, "Wrong hex '%'(%) for signature(65/130)(RSV)", hex, hex.size());
    eosio::ecc_signature ecc_sig;
    for (auto i = 0; i < 130; i += 2) {
        auto f = int_by_hex(hex[i]);
        auto s = int_by_hex(hex[i + 1]);
        ds_assert(f != -1 && s != -1, "Wrong hex '%'(%) for signature, it contains wrong char pos:'%' or pos:'%'.",
                  hex, hex.size(), i, i + 1);
        auto idx = i / 2;
        auto v = f * 16 + s;
        if (idx == 64) ecc_sig[0] = v;
        if (idx < 64) ecc_sig[idx + 1] = v;
        //ds_print("\r\n %[%-%] = %[%]*16+%[%] ",v,i/2,i,int_by_hex(hex[i]),(char)hex[i],int_by_hex(hex[i+1]),(char)hex[i+1]);
        //if(idx>=1 && idx<=32)idx = 33-idx;
        //if(idx>=33 && idx<=65)idx =  97 - idx;
    }
    return eosio::signature{std::in_place_index < 0 > , ecc_sig};
    //auto V = (ecc_sig.data()[0] - 35)%2;
    //ecc_sig[0]=V+27+4;
}